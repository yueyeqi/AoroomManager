...
///  webView 请求
    func loadWebView() {
        webView.loadRequest(URLRequest(url: URL(string: "http://www.aoroom.com/address.htm")!))
    }
    ...
    func handleLongTouch(){
        if(PersonalViewController.imgUrl != "" && touchState == TouchState.start){
            ...
            alertSheetCtr.addAction(UIAlertAction(title: "保存图片", style: .default, handler: { (alert) in
                let urlToSave: String = self.webView.stringByEvaluatingJavaScript(from: PersonalViewController.imgUrl)!
