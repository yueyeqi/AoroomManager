...
class AgentViewController: BaseViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.navigationController?.title = "代理入口"
...
///  webView 请求
    func loadWebView() {
        webView.loadRequest(NSURLRequest(URL: NSURL(string: "http://www.aoroom.com.au/code.html")!))
    }
...
// MARK: - Action
    ...
    @IBAction func backAction(sender: AnyObject) {
        if currenUrl == "http://www.aoroom.com.au/code.html" {
            return
        }
        self.webView.goBack()
    }
    ...
    func handleLongTouch(){
        if(AgentViewController.imgUrl != "" && touchState == TouchState.Start){
            ...
            alertSheetCtr.addAction(UIAlertAction(title: "保存图片", style: .Default, handler: { (alert) in
                let urlToSave: String = self.webView.stringByEvaluatingJavaScriptFromString(AgentViewController.imgUrl)!
                if let data: NSData = NSData(contentsOfURL: NSURL(string: urlToSave)!)! {
...
extension AgentViewController: UIWebViewDelegate {
    func webViewDidStartLoad(webView: UIWebView) {
        ...
        if str?.characters.count > 0 {
            ...
            self.navigationController!.navigationBar.topItem!.title = "代理入口"
        }
            ...
            if(components[1] == "touch"){
                if(components[2] == "start"){
                    ...
                    if(tagName!.uppercaseString == "IMG")
                    {
                        ...
                        AgentViewController.imgUrl = srcJS
                        if(AgentViewController.imgUrl != ""){
...
extension AgentViewController: NJKWebViewProgressDelegate {
    func webViewProgress(webViewProgress: NJKWebViewProgress!, updateProgress progress: Float) {
        webViewProgressView?.setProgress(progress, animated: true)
    }
}

extension AgentViewController: shareViewDelegate {
