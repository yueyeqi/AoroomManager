...
class HomeWebViewController: BaseViewController {
    ...
    ///  webView 请求
    func loadWebView() {
        webView.loadRequest(NSURLRequest(URL: NSURL(string: "http://aoroom.com/test6.html")!))
    }
    ...
    @IBAction func backAction(sender: AnyObject) {
        if currenUrl == "http://aoroom.com/test6.html" {
            return
        }
        self.webView.goBack()
    }
    ...
    func handleLongTouch(){
        if(HomeWebViewController.imgUrl != "" && touchState == TouchState.Start){
            ...
            alertSheetCtr.addAction(UIAlertAction(title: "保存图片", style: .Default, handler: { (alert) in
                let urlToSave: String = self.webView.stringByEvaluatingJavaScriptFromString(HomeWebViewController.imgUrl)!
...
extension HomeWebViewController: UIWebViewDelegate {
            ...
            if(components[1] == "touch"){
                if(components[2] == "start"){
                    ...
                    if(tagName!.uppercaseString == "IMG")
                    {
                        ...
                        HomeWebViewController.imgUrl = srcJS
                        if(HomeWebViewController.imgUrl != ""){
...
extension HomeWebViewController: NJKWebViewProgressDelegate {
    func webViewProgress(webViewProgress: NJKWebViewProgress!, updateProgress progress: Float) {
        webViewProgressView?.setProgress(progress, animated: true)
    }
}

extension HomeWebViewController: shareViewDelegate {
